...
REPO = BASE.parents[3]
MEDIA = BASE.parent / "media/test-02"
AUDIO = BASE.parent / "media/test-01/test01-original-c.wav"
    ...
    if mode in ("preflight", "submit"):
        ...
        if mode == "preflight":
            ...
            save("SYNC-SUBMISSION-INTENT.json", {"at": now(), "model": MODEL,
                 "scope": value.get("scope", "Authorized SELFIE-AVATAR-002 test-02 near-frontal seven-second test"),
        ...
        data = fetch(url)
        path = MEDIA / "test02-original-sync.mp4"
        if path.exists():
