...
Current outcome: the owner rejected the seven-second InfiniteTalk result as worse than HeyGen. HeyGen also remains unaccepted because of the reported lips and perceived voice issues. The accepted Seedance plus Sync test-02 remains the quality baseline. Both authorized comparison jobs are complete; this feedback does not request a retry, repair or full-take generation. The estimates and recommendations below retain the earlier comparison history and are superseded by this result where they propose an unperformed test. Current billing reconciliation is in cost-audit-2026-09-13/.
...
Owner correction: fresh looks every week, with an expanding imagegen library retained for future use. See reference/LOOK-LIBRARY.json, initially indexing the accepted home/olive and office/polo references. For recurring production, lock the exact script and voice first, select or create the week’s look, generate from the approved audio, review the full spoken take, and only then apply the existing Counterproof captions. Avoid routine full-video lip-sync repair when an audio-driven model already passes review. Keep Seedance plus Sync as the proven short-test fallback until an alternative passes.

Live account verification now supersedes the earlier signed-out browser limitation: Chrome is signed in. The settled subscription table shows nine active entries covering eight unique image models, no video models. See WEB-UNLIMITED-2026-09-13.json. Nano Banana 2 and Pro 2K expire September 18; six image-model entries display Auto-renewing. The current Seedance 2.5 720p offer is $120 for one day, $280 for three or $540 for seven, with one concurrent generation and clips up to 30 seconds. No purchase was submitted. Web Unlimited is intended for manual human use and excludes automation, MCP/CLI, Canvas and Supercomputer; it cannot be used as an automated browser workaround. Switching models alone does not create a free video entitlement.
...
The owner subsequently approved the exact two-provider seven-second test. Existing HeyGen CLI authentication bills the Creator subscription, not the separate API cash balance. Read-only live account showed 557 credits before submission and 553 afterward; record the observed four-credit decrease rather than the earlier approximately two-credit estimate. No new cash purchase or credential change occurred. The seven-second HeyGen result is complete and locally verified; per-video billing detail remains unavailable, so the four-credit figure is explicitly an observed account debit. HeyGen remains unaccepted. WaveSpeed InfiniteTalk then completed the identical seven-second sample for an observed $0.42 debit. HeyGen has owner concerns about lips and perceived voice; InfiniteTalk is rejected for this test because the owner says it is worse than HeyGen. The accepted Seedance plus Sync test-02 remains the quality baseline. Provider receipts are in test-03/.
