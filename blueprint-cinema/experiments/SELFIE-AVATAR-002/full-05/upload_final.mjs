...
const source=path.join(base,'assembly/renders/week2-full05-closing.mp4');
...
const qa=JSON.parse(fs.readFileSync(path.join(base,'ASSEMBLY-QA.json'),'utf8'));
if(qa.output_sha256!==sha256 || qa.status!=='passed' || !qa.accepted_decoded_picture_and_timing_preserved || !qa.original_audio_packets_and_pcm_preserved)throw Error('Final preservation QA does not bind this output.');
...
try{
 const file_url=await fal.storage.upload(new File([bytes],'selfie-week2-full05-'+sha256.slice(0,16)+'.mp4',{type:'video/mp4'}));
