"""One authorized Original C Sync v3 restoration per full-01 section.

Usage: python3 restore_sections.py {middle,closing} {preflight,submit,status,result,download}
INPUT.json lives in full-01/<section>/ and requires video_url, video_sha256,
...
RANGES = {"middle": (291590, 1048000), "closing": (1048000, 1865770)}
BASE = None
SECTION = None
...
def validate_input(remote=False):
    ...
    require(manifest["source_sha256"] == MASTER_SHA and manifest["sample_rate"] == 48000,
            "Authoritative audio manifest changed source identity or sample rate")
    sections = [item for item in manifest["sections"] if item["name"] == SECTION]
    require(len(sections) == 1, "Audio section is missing or ambiguous")
    audio = sections[0]
    require(tuple(audio["source_samples"]) == RANGES[SECTION], "Authoritative source sample range changed")
    ...
    require(isinstance(value["duration"], (int, float)) and not isinstance(value["duration"], bool)
            ...
            and abs(audio["duration"] - frames / 48000) < 1e-8, "Duration differs from exact authorized audio samples")
    ...
    wav = exact_local(value["audio_local_path"], MEDIA / (SECTION + "-original-c.wav"))
    ...
    request = {"model": MODEL, "section": SECTION,
...
def main():
    ...
    parser.add_argument("section", choices=tuple(RANGES))
    ...
    if mode in ("preflight", "submit"):
        ...
        if mode == "preflight":
            ...
            save("SYNC-SUBMISSION-INTENT.json", {"at": now(), "model": MODEL, "section": SECTION,
                 "scope": value.get("scope", "Authorized SELFIE-AVATAR-002 full-01 " + SECTION + " restoration"),
