...
name=sys.argv[1]
assert name in ('middle','closing','final')
if name=='final':
    source=BASE/'assembly/public/audio/original-c.wav'
    video=BASE/'assembly/renders/week2-full01-uncaptioned.mp4'
    expected='0ad39c68a67b6c0f11edcbbe952380fc616716a59788dc049e2613313e4091e0'
    report_path=BASE/'FINAL-AUDIO-QA.json'
else:
    source=BASE/'media'/f'{name}-original-c.wav'
    video=BASE/'media'/f'{name}-restored.mp4'
    expected=next(x['sha256'] for x in json.loads((BASE/'AUDIO-INPUTS.json').read_text())['sections'] if x['name']==name)
