"""One whole W2 guide then one Original C transfer; immutable input, no retries."""
...
def main():
    ...
    style = json.loads((BASE / 'W2.style.json').read_text())['style_instructions']
    assert len(text.encode()) <= 4000 and len(style.encode()) <= 4000
    guide = MEDIA / 'voice-w2.guide.wav'
    final = MEDIA / 'voice-w2.original-c.wav'
    request = cal.guide_body(text, style, capture.GUIDE_VOICE)
    if stage == 'prepare':
        save(folder / 'INPUT.json', {
            'scope':'One full authorized W2 guide plus one Original C transfer; no segmentation, processing, retry or canonical edit',
    ...
    if stage == 'transfer':
        ...
        save(folder/'TRANSFER-INPUT.json',{'at':now(),'script_sha256':EXPECTED_SHA,
            ...
            'scope':'One Original C transfer of the independently transcribed complete W2 guide'})
    ...
    if stage == 'guide':
        ...
        raw=MEDIA/'voice-w2.google-response.bin'
        ...
        raw=MEDIA/'voice-w2.elevenlabs-response.pcm'
        with raw.open('xb') as output: output.write(body)
