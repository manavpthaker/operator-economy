"""One whole W2B guide then one Original C transfer; immutable input, no retries."""
...
MEDIA = REPO / 'blueprint-cinema/experiments/SELFIE-AVATAR-002/media/voice'
sys.path.insert(0, str(REPO / 'operator-blueprint-v2/02-narration-production/tools'))
import capture_n4b as capture
import calibrate as cal
EXPECTED_SHA = '0434553ff578eb611f7764a93ad8bc1df1e3942ee29fd400917fb23a8fbf91bf'
...
def main():
    ...
    style = json.loads((BASE / 'W2B.style.json').read_text())['style_instructions']
    assert len(text.encode()) <= 4000 and len(style.encode()) <= 4000
    guide = MEDIA / 'voice-w2b.guide.wav'
    final = MEDIA / 'voice-w2b.original-c.wav'
    request = cal.guide_body(text, style, capture.GUIDE_VOICE)
    if stage == 'prepare':
        save(folder / 'INPUT.json', {
            'scope':'One full authorized W2B guide plus one Original C transfer; no segmentation, processing, retry or canonical edit',
            'script_path':str(SCRIPT.relative_to(REPO)), 'script_sha256':EXPECTED_SHA,
            'text':text, 'word_count':130, 'style_sha256':sha(style.encode()), 'guide_request':request,
        ...
        print(json.dumps({'stage':stage,'status':'prepared','words':130}),flush=True)
        return
    ...
    if stage == 'transfer':
        ...
        save(folder/'TRANSFER-INPUT.json',{'at':now(),'script_sha256':EXPECTED_SHA,
            ...
            'scope':'One Original C transfer of the independently transcribed complete W2B guide'})
    ...
    if stage == 'guide':
        ...
        raw=MEDIA/'voice-w2b.google-response.bin'
        ...
        raw=MEDIA/'voice-w2b.elevenlabs-response.pcm'
        with raw.open('xb') as output: output.write(body)
