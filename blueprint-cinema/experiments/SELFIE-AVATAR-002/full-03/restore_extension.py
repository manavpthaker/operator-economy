"""One authorized Original C Sync v3 restoration for the full-03 forward extension.

Usage: python3 restore_extension.py {preflight,submit,status,result,download}
INPUT.json lives in full-03/extension/ and requires video_url, video_sha256,
...
RANGES = {"extension": (1048000, 1865770)}
AUDIO_SHA = "acc826a4d3f711309a4fda79566abce81c1f9a06d6d628cabe466d71947cd6dd"
AUDIO_URL = "https://d2ol7oe51mr4n9.cloudfront.net/user_3J3m5xtqP8Xv0MOsPutf0uV3maX/60e6ce73-7e16-4126-a2f7-9401ac1cae98.mp3"
NATIVE_JOB = "2c989ce7-f3b4-449f-998f-6b8396691221"
BASE = None
SECTION = "extension"
...
def validate_input(remote=False):
    ...
    manifest_raw = (FULL / "AUDIO-INPUTS.json").read_bytes()
    manifest = json.loads(manifest_raw)
    require(isinstance(manifest, dict) and isinstance(manifest.get("sections"), list)
            and all(isinstance(item, dict) for item in manifest["sections"]),
            "Authoritative audio manifest must contain a sections list")
    require(manifest.get("source_sha256", MASTER_SHA) == MASTER_SHA
            and manifest.get("sample_rate", 48000) == 48000,
            "Authoritative source identity or sample rate changed")
    sections = [item for item in manifest["sections"] if item.get("name") == SECTION]
    require(len(sections) == 1, "Audio section is missing or ambiguous")
    audio = sections[0]
    require(audio["sha256"] == AUDIO_SHA and audio["url"] == AUDIO_URL,
            "Authoritative extension audio hash or hosted URL changed")
    frames = RANGES[SECTION][1] - RANGES[SECTION][0]
    require(tuple(audio.get("source_samples", RANGES[SECTION])) == RANGES[SECTION],
            "Authoritative extension source sample range changed")
    require(abs(audio.get("duration", frames / 48000) - frames / 48000) < 1e-8,
            "Manifest duration differs from exact authorized samples")
    ...
    wav = exact_local(value["audio_local_path"], MEDIA / "extension.wav")
    ...
    with wave.open(str(wav), "rb") as stream:
        ...
        extension_pcm = stream.readframes(frames)
    ...
    with wave.open(str(master), "rb") as stream:
        ...
        stream.setpos(RANGES[SECTION][0])
        require(stream.readframes(frames) == extension_pcm,
                "Extension must preserve approved master samples [1048000,1865770) exactly")
    ...
    require((BASE / "INPUT.json").read_bytes() == raw and (FULL / "AUDIO-INPUTS.json").read_bytes() == manifest_raw,
...
def job():
    ...
    require(request["input_record_sha256"] == sha(bound_input) == intent["input_record_sha256"]
            ...
            and sha((FULL / "AUDIO-INPUTS.json").read_bytes()) == request["audio_manifest_sha256"],
    ...
    if mode in ("preflight", "submit"):
        ...
        if mode == "preflight":
            ...
            save("SYNC-SUBMISSION-INTENT.json", {"at": now(), "model": MODEL, "section": SECTION,
                 "scope": value.get("scope", "Authorized SELFIE-AVATAR-002 full-03 " + SECTION + " restoration"),
