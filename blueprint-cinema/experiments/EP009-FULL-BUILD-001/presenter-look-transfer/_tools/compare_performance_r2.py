...
def probe(path):
    args = ['ffprobe', '-v', 'error', '-select_streams', 'v:0',
            '-show_entries', 'stream=width,height,r_frame_rate,avg_frame_rate,start_time,duration,nb_frames,time_base:format=duration:frame=best_effort_timestamp,duration,pkt_duration',
            '-show_frames', '-of', 'json', str(path)]
    raw = json.loads(subprocess.check_output(args))
    stream = raw['streams'][0]
    time_base = Fraction(stream['time_base'])
    pts = np.array([float(int(f['best_effort_timestamp']) * time_base) for f in raw['frames']])
    if len(pts) < 2 or np.any(np.diff(pts) <= 0):
        raise ValueError('Need strictly increasing decoded video PTS: ' + str(path))
    delta = np.diff(pts)
    last = raw['frames'][-1]
    tail_ticks = last.get('duration', last.get('pkt_duration'))
    tail = float(int(tail_ticks) * time_base) if tail_ticks is not None else float(np.median(delta))
    if tail <= 0:
    ...
    return {'stream': stream, 'format_duration_s': float(raw['format']['duration']),
            ...
            'timestamp_method': 'Integer best_effort_timestamp and duration ticks multiplied by exact rational stream time_base; no six-decimal printed timestamp arithmetic.'}
...
def self_test():
    ...
    source = np.arange(288) / 24
    candidate = np.arange(281) / 24
    pairs = pair_timestamps(source, candidate, float(Fraction(281, 24)))
    assert np.flatnonzero(pairs < 0).tolist() == list(range(281, 288))
    assert pairs[:281].tolist() == list(range(281))
