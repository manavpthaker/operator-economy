"""One authorized A-only LatentSync comparison; immutable receipts, no automatic retries."""
import datetime, hashlib, importlib.util, json, subprocess, sys
from pathlib import Path
sys.dont_write_bytecode = True
D = Path(__file__).resolve().parents[1]
R = next(p for p in D.parents if (p / '.agents').is_dir())
G = D / 'repair-latentsync-r1'; A = G / 'P08r3a'; F = A / 'fal'
MODEL = 'fal-ai/latentsync'
ITEM = 'look-transfer-P08r3a-fal-latentsync-repair-r1'
...
def submit():
    ...
    payload = {'video_url':vu, 'audio_url':au, 'guidance_scale':1.0, 'seed':90081}
    ...
    intent = {'at':now(), 'model':MODEL, 'payload':payload, 'plan':bind(G/'PLAN.json'),
              ...
              'rate':'$0.20 per video up to40seconds', 'rate_source':plan['rate_source'], 'no_retry':True}
...
def result():
    ...
    r.ledger({'item':ITEM, 'provider':'fal', 'model':MODEL, 'status':'done', 'actual_credits':0,
              ...
              'reason':'Completed; posted $0.20 flat charge for up to40seconds, not provider invoice', 'plan':bind(G/'PLAN.json')})
