...
for s in scenes:
    ...
    dr.text((4, 3), f"EP009 {s['scene']}  {s['segs'][0]['id']}-{s['segs'][-1]['id']}  frames {s['f0']}-{s['f1']}  1 fps  r1", fill=(255, 220, 80), font=fnt)
    for k in range(n):
        f = frames[k]
    ...
    p = f"{A}/qa/contact-r1/{s['scene']}.png"
    sheet.save(p)
