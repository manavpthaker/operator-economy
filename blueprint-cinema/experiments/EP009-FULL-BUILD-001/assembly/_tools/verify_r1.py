...
OUT = A + '/qa/ep009-full-r1.mp4'
FPS = 24
W, H = 1280, 720
SW, SH = 320, 180

plan = json.load(open(B + '/direction/SHOT-PLAN.json'))
rows = json.load(open(A + '/_tools/sources-r1.json'))
...
ver['output'] = {
    'path': 'assembly/qa/ep009-full-r1.mp4', 'sha256': sha(OUT),
...
# ---------------- seams ----------------
os.makedirs(A + '/qa/seams-r1', exist_ok=True)
...
for i, (a, b) in enumerate(zip(rows, rows[1:])):
    ...
    pa = f'{A}/qa/seams-r1/{name}-a.png'
    pb = f'{A}/qa/seams-r1/{name}-b.png'
    ...
    seams.append({
        ...
        'png_a': f'assembly/qa/seams-r1/{name}-a.png', 'png_b': f'assembly/qa/seams-r1/{name}-b.png',
        'flags': flags,
    })
ver['seams'] = {
    'method': 'Last outgoing and first incoming frames taken by frame index from the full decode of the encoded ep009-full-r1.mp4; difference = mean absolute RGB difference in 0-255 at 1280x720; half-drawn heuristic = incoming frame gray std under 0.6x the std 12 frames later with more than 10/255 change.',
...
os.makedirs(A + '/qa/sync-r1', exist_ok=True)
sync = []
fnt = font(18)
for take, sid in [('P01', 'seg009'), ('P03', 'seg019'), ('P05', 'seg028')]:
    ...
    d = f"{A}/qa/sync-r1/{take}-{sid}-first-word-{w0['w_id']}-{re.sub(r'[^A-Za-z]', '', w0['token'])}-onset-{w0['start']:.3f}s"
    ...
    sync.append({'take': take, 'segment': sid, 'first_word': w0, 'window_s': [round(t0, 3), round(t0 + 4, 3)],
                 'frames_dir': os.path.relpath(d, B + '/..').replace('EP009-FULL-BUILD-001/', 'blueprint-cinema/experiments/EP009-FULL-BUILD-001/', 1) if False else 'assembly/qa/sync-r1/' + os.path.basename(d),
                 'strip': 'assembly/qa/sync-r1/' + os.path.basename(strip), 'clip_with_sound': 'assembly/qa/sync-r1/' + os.path.basename(clip),
...
json.dump(ver, open(A + '/_tools/verify_partial-r1.json', 'w'), indent=1)
