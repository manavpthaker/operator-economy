...
@main struct Capture {
    static func main() async throws {
        // ScreenCaptureKit's window inventory is backed by the AppKit graphics session.
        // Initializing an application object makes this command-line recorder usable from
        // the local production shell without automating another application's UI.
        _ = NSApplication.shared
        let args = CommandLine.arguments
        let content = try await SCShareableContent.excludingDesktopWindows(true, onScreenWindowsOnly: false)
        let windows = content.windows.filter { $0.owningApplication?.bundleIdentifier == "com.google.Chrome" && ($0.title ?? "").contains("EP009") }
        if args.count < 4 {
            for w in windows { print("\(w.windowID)\t\(w.frame)\t\(w.title ?? "")") }
            return
        }
        guard let id = UInt32(args[1]), let window = windows.first(where: {$0.windowID == id}) else {
            throw NSError(domain: "EP009Capture", code: 1, userInfo: [NSLocalizedDescriptionKey:"Selected EP009 Chrome window is not available"])
        }
        ...
        guard delegate.finished else { throw NSError(domain:"EP009Capture", code:2, userInfo:[NSLocalizedDescriptionKey:"Recording finalization timeout"]) }
    }
}
