...
report={'output':str(out.relative_to(P)),'sha256':sha(out),'duration':probe['format']['duration'],'dimensions':[v['width'],v['height']],'frames':v['nb_frames'],'fps':v['r_frame_rate'],'audio_comparison':{'reference':'assets/voice.wav','samples':n,'correlation':dot/math.sqrt(aa*bb),'gain':dot/aa,'note':'Encoded AAC compared to exact locked PCM concatenation; similarity is not perceptual mouth-sync review.'},'source_contract_sha256':sha(P/'source-contract.json'),'check_sha256':sha(P/'check-final.json'),'presenter_review':'Source frame mappings preserve exact accepted performance; no regeneration, freeze or retime. First source retains its existing close-to-wide cut and matching gestures; second source keeps both hands in the portrait crop. No new performance or timing change.','publication':'private_review_only'}
