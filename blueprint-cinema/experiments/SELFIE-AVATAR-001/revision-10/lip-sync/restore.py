"""One new revision-10 whole-script global restoration; no previous revision inputs or intents."""
import datetime
import hashlib
import json
import re
...
def api(url, payload=None):
    ...
    try:
        ...
        raise SystemExit('Fal returned an error; inspect revision-10/lip-sync/ERROR.json')
...
def main():
    ...
    if mode == 'preflight':
        ...
        data = fetch(url)
        path = MEDIA / 'home-olive-gtm-r10-sync.mp4'
        if path.exists():
