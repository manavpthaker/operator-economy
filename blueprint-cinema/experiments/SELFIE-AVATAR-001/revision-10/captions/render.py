"""R10 full-script captions using the current Counterproof heading treatment.

Burn only a freshly verified SRT bound to this exact clean video and script.
The installed subtitle burner preserves the heading design and copied audio.
No earlier revision's word timings, cue counts, or voice map are reused.
"""
import base64
import hashlib
import io
...
contact_indices = [round(j*(cue_count-1)/3) for j in range(4)]
contact_samples = []
contact = Image.new('RGB',(1440,1280),'#202426')
for j,index in enumerate(contact_indices):
    start,end,text = cues[index]
    contact_samples.append({'cue_index':index+1,'midpoint_seconds':(start+end)/2,'text':text})
