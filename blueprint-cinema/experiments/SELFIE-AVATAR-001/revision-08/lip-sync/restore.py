...
REPO = BASE.parents[4]
MEDIA = REPO / "blueprint-cinema/experiments/SELFIE-AVATAR-001/media/revision-08"
MODEL = "fal-ai/sync-lipsync/v2/pro"
SYNC_MODE = "cut_off"
OUTPUT = MEDIA / "home-olive-gtm-r8-opening-sync.mp4"
...
def fetch(url):
    request = urllib.request.Request(https_url(url), headers={"User-Agent": "OE-R8-integrity-check"})
...
def validate(incoming, hosted):
    require(incoming["section_index"] == 1, "Only the R8 opening is permitted")
