"""R8 media-sandbox-only trim and frame QA for one opening."""
...
native_count=int(v['nb_frames']); tail_pad=max(0,n-native_count)
assert tail_pad<=5, 'Only the observed five-frame ending shortfall may be padded'
vf=f'trim=start_frame=0:end_frame={min(n,native_count)},setpts=PTS-STARTPTS'
if tail_pad:vf+=f',tpad=stop_mode=clone:stop_duration={tail_pad/24},trim=end_frame={n}'
run(['ffmpeg','-v','error','-y','-i','native.mp4','-vf',vf,'-an','-c:v','libx264','-preset','fast','-crf','15','-pix_fmt','yuv420p','-tag:v','avc1','-r','24','-movflags','+faststart','picture.mp4'])
...
vr=put('picture.mp4',c['slots'][0]);vr.update({'frame_count':n,'frame_rate':'24/1','width':720,'height':1280,'duration_seconds':n/24,'audio_stream_count':0,'local_path':'blueprint-cinema/experiments/SELFIE-AVATAR-001/media/revision-08/picture.mp4'})
...
report={'video':vr,'native':{'url':c['native_url'],'sha256':sha('native.mp4'),'probe':probe},'matched_probe':p,'tail_clone_frames':tail_pad,'tail_clone_seconds':tail_pad/24,'tail_note':'Pad only the terminal visual hold to the unchanged R7 voice duration; no speech time scaling.', 'contact':ir,'limits':'Still frames describe facial shape only; no synchronized perceptual lip-sync verdict.'}
