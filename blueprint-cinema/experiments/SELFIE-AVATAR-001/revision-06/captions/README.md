# R6 Counterproof heading captions

Prepared adapter only. No R6 caption render or delivery has run from this folder.

`render.py` preserves the prior highlighted caption renderer from
`revision-05/captions/render.py`, preserving its R4 heading treatment: Archivo 56 px, weight 630,
...
The input must be the new clean R6 assembly after the three duration-matched
native-shot Sync passes. The complete R4 narration and its relative word clock
remain fixed. A zero offset is possible but must not be assumed or hardcoded. Before
running, freshly compare the decoded R6 audio with the full R4 voice, confirm
...
and font have fixed SHA-256 pins inside the adapter. R4 and R5 files stay untouched.
...
| Key | Value |
|---|---|
| `video_url`, `video_sha256` | New uncaptioned R6 assembled video and exact hash |
...
| `audio_offset_seconds` | Fresh measured offset from full R4 voice to the final R6 assembly |
