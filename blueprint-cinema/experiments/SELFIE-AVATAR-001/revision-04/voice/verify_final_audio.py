...
'words':words,'duration_seconds':duration,'engine':'faster-whisper small.en; beam_size=5; no initial prompt',
...
for label,ending in [('after_word_68','already.'),('after_word_124','fine.')]:
 ...
 if candidates:
  ...
  bounds.append({'label':label,'status':'quiet_candidate' if quiet else 'lowest_energy_candidate','previous':previous,'next':nextword,'safe_ASR_gap':[lo,hi],'selected':choice})
