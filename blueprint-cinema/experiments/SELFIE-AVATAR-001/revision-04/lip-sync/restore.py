"""One new revision-04 whole-script global restoration; no previous revision inputs or intents."""
import datetime
import hashlib
import json
import re
...
def api(url, payload=None):
    ...
    try:
        ...
        raise SystemExit('Fal returned an error; inspect revision-04/lip-sync/ERROR.json')
...
def main():
    ...
    if mode == 'preflight':
        ...
        data = fetch(url)
        path = MEDIA / 'home-olive-gtm-r4-sync.mp4'
        if path.exists():
