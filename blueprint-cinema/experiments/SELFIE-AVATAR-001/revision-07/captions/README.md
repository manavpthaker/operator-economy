# R7 first-section heading captions

Rendered and uploaded by root. See REPORT.json, TIMING-MAP.json and ../DELIVERY.json for the completed sample and current review limits.
...
frames at 24 fps. `../voice/R7-VOICE-REPORT.json` has SHA-256
...
`render.py` keeps the R6 heading callback and burn arguments: Archivo 56 px,
...
sample video. A zero offset is not assumed.
...
- `video_url`, `video_sha256`: clean R7 sample and exact hash.
...
- `audio_offset_seconds`: fresh measured offset from the edited R7 voice to the clean sample.
...
R4–R6 sources remain unchanged. No completed render, owner acceptance, lip-sync
