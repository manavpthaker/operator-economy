"""One new revision-03 voice-R3 restoration; no old seven-second inputs or intents."""
import datetime
import hashlib
import json
import re
...
def api(url, payload=None):
    ...
    try:
        ...
        raise SystemExit('Fal returned an error; inspect revision-03/lip-sync/ERROR.json')
...
def main():
    ...
    if mode == 'prepare':
        ...
        data = fetch(url)
        path = MEDIA / 'home-olive-r3-sync.mp4'
        if path.exists():
