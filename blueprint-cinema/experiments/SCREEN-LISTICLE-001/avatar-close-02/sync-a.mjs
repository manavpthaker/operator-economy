...
const video=fs.readFileSync(path.join(base,'media/native-a.mp4')), audio=fs.readFileSync(path.join(base,'media/take-a.wav'));
const video_url=await fal.storage.upload(new File([video],'close02a-native-'+sha(video).slice(0,12)+'.mp4',{type:'video/mp4'}));
const audio_url=await fal.storage.upload(new File([audio],'close02a-audio-'+sha(audio).slice(0,12)+'.wav',{type:'audio/wav'}));
const input={video_url,audio_url,sync_mode:'silence'};
save('SYNC-a-REQUEST.json',{at:new Date().toISOString(),model:'fal-ai/sync-lipsync/v3',input,video_sha256:sha(video),audio_sha256:sha(audio)});
const result=await fal.subscribe('fal-ai/sync-lipsync/v3',{input,logs:false});
save('SYNC-a-RESULT.json',{at:new Date().toISOString(),request_id:result.requestId,data:result.data});
const url=result.data?.video?.url; if(!url) throw Error('no video url');
const out=Buffer.from(await (await fetch(url)).arrayBuffer());
fs.writeFileSync(path.join(base,'media/synced-a.mp4'),out,{flag:'wx'});
