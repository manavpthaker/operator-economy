...
const video=fs.readFileSync(path.join(base,'media/native.mp4')), audio=fs.readFileSync(path.join(base,'media/close-audio.wav'));
const video_url=await fal.storage.upload(new File([video],'close01-native-'+sha(video).slice(0,12)+'.mp4',{type:'video/mp4'}));
const audio_url=await fal.storage.upload(new File([audio],'close01-audio-'+sha(audio).slice(0,12)+'.wav',{type:'audio/wav'}));
