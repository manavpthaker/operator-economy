...
const input={video_url:req0.input.video_url,audio_url:req0.input.audio_url,sync_mode:'cut_off'};
const model=process.argv[2]||'fal-ai/sync-lipsync/v2/pro';
fs.writeFileSync(path.join(base,'SYNC2PRO-REQUEST.json'),JSON.stringify({at:new Date().toISOString(),model,input},null,2)+'\n',{flag:'wx'});
const result=await fal.subscribe(model,{input,logs:false});
fs.writeFileSync(path.join(base,'SYNC2PRO-RESULT.json'),JSON.stringify({at:new Date().toISOString(),request_id:result.requestId,data:result.data},null,2)+'\n',{flag:'wx'});
const url=result.data?.video?.url; if(!url) throw Error('no video url');
const out=Buffer.from(await (await fetch(url)).arrayBuffer());
fs.writeFileSync(path.join(base,'media/synced-2pro.mp4'),out,{flag:'wx'});
