"""Execute only the pinned, owner-authorized R23 take. Never retry a submission."""
...
assert P['model']==A['model']=='fal-ai/infinitalk'
assert sha((ROOT/'PROPOSED-REQUEST.json').read_bytes())==A['proposal_sha256']
assert P['expected_cost_usd']<=A['estimated_usd_ceiling']==7
for kind in ('image','audio','original_audio'):
    assert sha((ROOT/P[kind+'_local_path']).read_bytes())==P[kind+'_sha256']
with wave.open(str(ROOT/P['audio_local_path']),'rb') as w:
    assert w.getframerate()==48000 and w.getnframes()==725760
assert P['parameters']['num_frames']==377 and 377%8==1 and 377/25<=15.12
...
def api(url,payload=None):
    ...
    req=urllib.request.Request(url,data=None if payload is None else json.dumps(payload).encode(),
        headers={'Authorization':'Key '+credential(),'Content-Type':'application/json','X-Fal-No-Retry':'1','x-app-fal-disable-fallback':'1'})
...
mode=sys.argv[1]
if mode=='verify':
    print(json.dumps({'local_inputs_verified':True,'authorized':True,'expected_usd':P['expected_cost_usd'],'ceiling_usd':7,'submission_attempt_exists':(ROOT/'SUBMISSION-ATTEMPT.json').exists()}))
elif mode=='upload':
    manifest=read('UPLOADS.json') if (ROOT/'UPLOADS.json').exists() else {}
    for kind,mime,ext in [('image','image/png','.png'),('audio','audio/wav','.wav')]:
        if kind in manifest:
            assert manifest[kind]['status']=='verified' and manifest[kind]['sha256']==P[kind+'_sha256']
            continue
        raw=(ROOT/P[kind+'_local_path']).read_bytes()
        manifest[kind]={'status':'initiating','sha256':sha(raw),'bytes':len(raw),'at':now()};save('UPLOADS.json',manifest)
        ack=api('https://rest.fal.ai/storage/upload/initiate?storage_type=fal-cdn-v3',{'content_type':mime,'file_name':'ep007-r23-'+kind+'-'+sha(raw)[:12]+ext})
        ...
        print(json.dumps({'input':kind,'status':'verified','sha256':sha(raw)}))
elif mode=='submit':
    uploads=read('UPLOADS.json');payload=dict(P['parameters'])
    for kind in ('image','audio'):
        assert uploads[kind]['status']=='verified' and uploads[kind]['sha256']==P[kind+'_sha256']
        payload[kind+'_url']=uploads[kind]['file_url']
    with (ROOT/'SUBMISSION-ATTEMPT.json').open('x') as f:
        json.dump({'started_at':now(),'model':A['model'],'submission_number':1,'automatic_retries':0,'proposal_sha256':A['proposal_sha256'],'authorization_sha256':sha((ROOT/'GENERATION-AUTHORIZATION.json').read_bytes()),'payload':payload,'payload_sha256':sha(json.dumps(payload).encode()),'estimated_usd':P['expected_cost_usd'],'may_retry':False},f,indent=2)
    ...
    save('JOB.json',job);print(json.dumps({'request_id':job['request_id'],'status':job.get('status'),'estimated_usd':P['expected_cost_usd']}))
