...
assert A['expected_usd']==P['proposed_limits']['nominal_estimated_usd']<=A['estimated_usd_ceiling']==2
...
with wave.open(str(ROOT/P['input_files']['audio']['local_path']),'rb') as w:
    assert (w.getframerate(),w.getnframes(),w.getnchannels(),w.getsampwidth())==(48000,412000,1,2)
...
if mode=='verify':
    credential()
    print(json.dumps({'inputs_and_authorization_verified':True,'credential_available':True,'expected_usd':A['expected_usd'],'ceiling_usd':2,'submission_attempt_exists':(ROOT/'SUBMISSION-ATTEMPT.json').exists()}))
elif mode=='upload':
    ...
    for kind,mime,ext in [('video','video/mp4','.mp4'),('audio','audio/wav','.wav')]:
        ...
        ack=api('https://rest.fal.ai/storage/upload/initiate?storage_type=fal-cdn-v3',{'content_type':mime,'file_name':'ep007-r25-'+kind+'-'+sha(raw)[:12]+ext})
