"""Bounded R69 replacement and conditional buyer operation. Never retry a paid submission."""
...
def upload(path,folder):
 ...
 ack=old.api('https://rest.fal.ai/storage/upload/initiate?storage_type=fal-cdn-v3',{'content_type':'image/png','file_name':'ep007-r69-'+sha(raw)[:16]+'.png'})
...
def main():
 ...
 assert auth['caps']=={'fal_total_usd':3.10,'requests_total_including_failed':3,'remaining_requests':2,'automatic_retries':0}
 assert sha((REPO/auth['prior_intent']['path']).read_bytes())==auth['prior_intent']['sha256']
 assert auth['prior_spend_usd']==0.896
 ...
 if mode=='submit':
  ...
  spent=auth['prior_spend_usd']+sum(read(p)['estimated_usd'] for p in BASE.glob('take-*/SUBMISSION-INTENT.json'))
  cost=int(req['input']['duration'])*.112
  assert len(list(BASE.glob('take-*/SUBMISSION-INTENT.json')))<2 and spent+cost<=3.10
