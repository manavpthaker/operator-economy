...
// R13 preserves source assets; only a new sketch and a silence-only source splice are authorized.
...
assert.equal(cursor,56.5);
const audio=clips.filter(c=>c.tag==='audio').sort((a,b)=>+a['data-start']-+b['data-start']);
assert.equal(audio.length,2);
assert.deepEqual(audio.map(c=>[+c['data-start'],+c['data-duration'],+c['data-media-start']]),[[0,45,0],[45,11.5,48]]);
assert(audio.every(c=>c.src==='public/audio/review-narration.wav'&&c['data-volume']==='1'&&!c['data-playback-rate']));
assert(masterPCM.subarray(45*48000*2,48*48000*2).every(v=>v===0),'removed material is not pure digital silence');
const assembledPCM=Buffer.concat(audio.map(c=>finalPCM.subarray(+c['data-media-start']*48000*2,(+c['data-media-start'] + +c['data-duration'])*48000*2)));
assert.equal(assembledPCM.length,2712000*2);
assert(assembledPCM.equals(Buffer.concat([masterPCM.subarray(0,45*48000*2),masterPCM.subarray(48*48000*2,59.5*48000*2)])));
assert.equal(await sha('../r12-logo-led/index.html'),'146b3c747e5fa84f33ac9a90c7ef797a3362fa3e9c3fb14b2e3b5988ff5ea65a');
...
const sourceFiles=['index.html','compositions/question.html','compositions/handoff.html','compositions/sting.html','compositions/title.html','BRIEF.md','frame.md','STORYBOARD.md'];
const authored=[];for(const path of sourceFiles)authored.push({path,sha256:await sha(path)});
const result={checked_at:new Date().toISOString(),ok:true,scope:'isolated assembled preview; not final rendered delivery',dimensions:[1280,720],fps:24,frames:1356,duration:56.5,checks:{source_pins:true,r12_preserved:true,film_copies_identical:true,staged_original_pcm_exact:true,retained_pcm_exact:true,removed_samples_all_zero:true,pickup_pcm_exact:true,one_gapless_audio_lane_all_picture_muted:true,frame_aligned_gapless_picture:true,source_ranges_in_bounds:true,browser_derivatives_8bit_preserve_frame_counts:true,subcomposition_contract:true},authored,timeline,audio_edit:{original_master_untouched:true,removed_source_seconds:[45,48],removed_samples:144000,output_samples:2712000,assembled_pcm_sha256:digest(assembledPCM),segments:audio.map(c=>({timeline_in:+c['data-start'],timeline_out:+c['data-start'] + +c['data-duration'],source_in:+c['data-media-start'],source_out:+c['data-media-start'] + +c['data-duration']})),source_to_review:'t<45: t; 45<=t<48: omitted; t>=48: t-3',introduction_review_time:45.52,logo_lead_before_speech_seconds:1.02},assets,provenance:{narration:master,narration_sha256:pins[0][1],retained_film_sources:sources,new_avatar:'None for R13; reuse prior Sync3 hosted-WAV recovery002',browser_derivatives:'Retained R12 8-bit silent derivatives, unchanged frame counts, no trim or retime',avatar_crop:'1280x724 top-anchored, clipped at1280x720; bottom-edge artifact removal',illustration:'Authored rough SVG worker, persistent job sheet, chair and bench; retained R12 owner pencil study. Single forearm offer and hard counterfactual absence, not equation/stroke spectacle. Illustrative dependency, not evidence.'}};
await writeFile(resolve(root,'ASSET-AND-EDIT-MANIFEST.json'),JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify({ok:result.ok,frames:1356,pictureClips:picture.length,assets:assets.length,pcm:'exact retained source samples; three seconds of zeros omitted',index_sha256:authored[0].sha256}));
