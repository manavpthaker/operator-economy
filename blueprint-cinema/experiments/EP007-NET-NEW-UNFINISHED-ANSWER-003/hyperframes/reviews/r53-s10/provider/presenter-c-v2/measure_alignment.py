"""Measure original-audio insertion before selecting R53 part C picture frames."""
from pathlib import Path
import json, subprocess, hashlib
import numpy as np
P=Path(__file__).resolve().parent
R=next(x for x in P.parents if (x/'.agents').exists())
plan=json.loads((R/'blueprint-cinema/experiments/EP007-NET-NEW-UNFINISHED-ANSWER-003/direction/r53-s10-schedule/presenter-c/AVATAR-TAKE-PLAN.json').read_text())
original=P/'audio/timing-not-yours.wav'
...
rows=[compare(.4,1.8),compare(3.5,5.5),compare(6.8,8.6)]
