"""R53 bounded Fal operation: one owner-plan take. Every paid intent is exclusive; never retry submission."""
...
DIRECTION = REPO / 'blueprint-cinema/experiments/EP007-NET-NEW-UNFINISHED-ANSWER-003/direction/r53-s10-schedule'
...
def upload(path, folder, mime):
    ...
    ack = api('https://rest.fal.ai/storage/upload/initiate?storage_type=fal-cdn-v3', {'content_type':mime,'file_name':'ep007-r53-'+sha(raw)[:16]+path.suffix})
...
def main():
    ...
    if mode == 'submit':
        ...
        with (folder/'SUBMISSION-INTENT.json').open('x') as f:
            json.dump({'at':now(),'model':p['model'],'payload':payload,'estimated_usd':0.896,'authorization_sha256':sha((BASE/'GENERATION-AUTHORIZATION-A-v2.json').read_bytes()),'no_retry':True}, f, indent=2)
