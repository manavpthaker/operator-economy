"""R53 part C authorized single-attempt restoration; no paid retries."""
...
PLAN=R/'blueprint-cinema/experiments/EP007-NET-NEW-UNFINISHED-ANSWER-003/direction/r53-s10-schedule/presenter-c/AVATAR-TAKE-PLAN.json'
CAP_USD=2.0
mode=sys.argv[1]
if mode=='upload-audio':
    a=P/'audio/timing-not-yours.wav'; assert sha(a)==read(PLAN)['audio']['sha256']
    ...
    with (out/'SUBMISSION-INTENT.json').open('x') as f:
        json.dump({'at':datetime.datetime.now(datetime.timezone.utc).isoformat(),'model':'fal-ai/sync-lipsync/v3','payload':payload,'estimated_usd':cost,'cap_usd':CAP_USD,'owner_statement':'approved both, build B and submit A and C','no_retry':True},f,indent=2)
