"""R53 bounded Fal operation: one owner-binder take. Every paid intent is exclusive; never retry submission."""
...
def main():
    ...
    if mode == 'submit':
        ...
        with (folder/'SUBMISSION-INTENT.json').open('x') as f:
            json.dump({'at':now(),'model':p['model'],'payload':payload,'estimated_usd':0.896,'authorization_sha256':sha((BASE/'GENERATION-AUTHORIZATION.json').read_bytes()),'no_retry':True}, f, indent=2)
