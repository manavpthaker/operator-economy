"""R52 bounded Fal operation: one v3 walk-out take. Every paid intent is exclusive; never retry submission."""
...
def upload(path, folder, mime):
    ...
    return ack['file_url']
def main():
    auth = read(BASE/'GENERATION-AUTHORIZATION-v3.json')
    assert auth['caps'] == {'fal_total_usd':1.25,'requests':1,'automatic_retries':0}
    reqpath = DIRECTION/'WALKOUT.request.json'
    assert sha(reqpath.read_bytes()) == auth['bound_request']['sha256'], 'Request changed since authorization'
    folder = BASE/'walkout-v3'; mode = sys.argv[1]
    if mode == 'submit':
        ...
        assert p['input']['duration'] == '10' and p['input']['generate_audio'] is False and 10*0.112 <= auth['caps']['fal_total_usd']
        ...
        with (folder/'SUBMISSION-INTENT.json').open('x') as f:
            json.dump({'at':now(),'model':p['model'],'payload':payload,'estimated_usd':1.12,'authorization_sha256':sha((BASE/'GENERATION-AUTHORIZATION-v3.json').read_bytes()),'no_retry':True}, f, indent=2)
