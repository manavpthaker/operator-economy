...
H=EXP/'hyperframes/reviews/r68-s22-film';C=EXP/'hyperframes/reviews/r68-s22-context'
...
cmd=['ffmpeg','-y','-v','error','-i',str(EXP/'hyperframes/reviews/r66-s21-hard-part/qa/s21.mp4'),'-i',str(H/'qa/s22.mp4'),'-i',str(MASTER),
...
with wave.open(str(MASTER),'rb') as src:src.setpos(25130*2000);expected=src.readframes(524*2000)
...
report={'status':'technical_pass_owner_review_pending','pcm_bit_exact':True,'strict_check':True,'runtime':'0.8.46','source_sha256':sha(H/'index.html'),
 'scene':inspect(H/'qa/s22.mp4',25130/24,524),'context':inspect(C/'qa/context.mp4',24938/24,716),
 'limits':['No owner acceptance or release.','Audio metrics are not a human listening verdict.','Film performance is recorded separately from structural checks.']}
(H/'qa/VERIFICATION.json').write_text(json.dumps(report,indent=2)+'\n')
frames=[191,192,246,247,309,310,360,420,479,480,550,610,670,715]
s='+'.join('eq(n\\,%s)'%n for n in frames)
run(['ffmpeg','-y','-v','error','-i',str(C/'qa/context.mp4'),'-vf',f'select={s},scale=640:360,tile=2x7','-frames:v','1','-update','1',str(H/'qa/encoded-contact.png')])
