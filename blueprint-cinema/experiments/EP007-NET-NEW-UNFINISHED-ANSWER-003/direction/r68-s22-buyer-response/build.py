...
H=EXP/'hyperframes/reviews/r68-s22-film'
C=EXP/'hyperframes/reviews/r68-s22-context'
...
for name in ['A','B']:
 ...
 assert sel['takes'][name]['out_frame']-sel['takes'][name]['in_frame']==(170 if name=='A' else 236)
...
for name in ['package.json','hyperframes.json']:
 text=(prior/name).read_text().replace('r67-s22-plan','r68-s22-film');(H/name).write_text(text)
...
<div id="root" data-composition-id="ep007-r68-s22" data-start="0" data-duration="21.833333333333332" data-width="1280" data-height="720" data-fps="24"><div class="back"></div>
'''
html+=video('table-return','establishing.mp4',0,55/24,4.5,1)+'\n'
html+=video('buyer-question','question.mp4',55/24,63/24,0,2)+'\n'
html+=video('owner-record','take-a.mp4',118/24,170/24,sel['takes']['A']['in_frame']/24,3)+'\n'
html+=video('buyer-response','take-b.mp4',12,236/24,sel['takes']['B']['in_frame']/24,4)+'\n'
