#!/usr/bin/env python3
"""Reproduce the bounded local S20 encode/source checks using FFmpeg and stdlib."""
...
SCENE = EXP / 'hyperframes/reviews/r65-s20-economics'
CONTEXT = EXP / 'hyperframes/reviews/r65-s20-context'
...
with wave.open(str(MASTER),'rb') as f:
    f.setpos(891*48000); expected=f.readframes(2274*2000)
...
report={'status':'technical_pass_owner_review_pending','master_sha256':sha(MASTER),'pcm_carve_bit_exact':True,'strict_check':True,'scene':inspect(SCENE/'qa/s20.mp4',891,94.75,2274),'context':inspect(CONTEXT/'qa/context.mp4',883,102.75,2466),'source_sha256':sha(SCENE/'index.html'),'limitations':['No owner acceptance; no audience comprehension claim.','Audio checks establish source alignment and level, not a human listening verdict.','Render and checks are local review output, not canonical picture lock or release.'],'runtime':{'version':'0.8.46','previous_copy_pin':'0.8.36','scope':'new S20 review only','strict_check_passed':True}}
