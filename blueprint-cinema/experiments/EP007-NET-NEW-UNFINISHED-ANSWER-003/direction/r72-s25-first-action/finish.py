...
H=EXP/'hyperframes/reviews/r72-s25-first-action';C=EXP/'hyperframes/reviews/r72-s25-context'
...
cmd=['ffmpeg','-y','-v','error','-i',str(EXP/'hyperframes/reviews/r71-s24-verdict/qa/s24.mp4'),'-i',str(H/'qa/s25.mp4'),'-i',str(MASTER),
 '-filter_complex','[0:v]trim=start_frame=352:end_frame=552,setpts=PTS-STARTPTS[v0];[1:v]trim=start_frame=0:end_frame=726,setpts=PTS-STARTPTS[v1];[v0][v1]concat=n=2:v=1:a=0[v];[2:a]atrim=start_sample=52472000:end_sample=54324000,asetpts=PTS-STARTPTS,pan=stereo|c0=c0|c1=c0[a]',
...
with wave.open(str(MASTER),'rb') as src:src.setpos(26436*2000);expected=src.readframes(726*2000)
...
report={'status':'technical_pass_owner_review_pending','pcm_bit_exact':True,'strict_check':True,'runtime':'0.8.46','source_sha256':sha(H/'index.html'),
 'scene':inspect(H/'qa/s25.mp4',26436/24,726),'context':inspect(C/'qa/context.mp4',26236/24,926),
...
frames=[0,96,199,200,323,346,402,467,486,578,585,591,680,716,828,925]
s='+'.join('eq(n\\,%s)'%n for n in frames)
run(['ffmpeg','-y','-v','error','-i',str(C/'qa/context.mp4'),'-vf',f'select={s},scale=640:360,tile=2x8','-frames:v','1','-update','1',str(H/'qa/encoded-contact.png')])
