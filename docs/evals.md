...
**2026-07-31 (eval precision pass — two false positives, both caught on real drafts):** The checks were failing correct scripts, which is worse than failing to catch bad ones, because it trains you to overrule the gate.

1. **Early-CTA kill-list fired on the word "subscriber."** `eval_package.py` tested `"subscribe" in early_text` — a bare substring match. EP004's evidence section says *"he had a multi-million-**subscribe**r audience before he sold a single seat"*, which is not a CTA, and the episode was kill-listed and marked publish-BLOCKED for it. Fixed with a word-boundary regex (`\b(?:subscribe|blueprint|link below|download)\b`). Verified that real CTAs still trip it and "subscriber"/"subscribers" no longer do. For a channel that covers audience businesses, this would have kept recurring.

2. **Hook-tension checks were digit-only, punishing VO-correct copy.** Voiceover spells numerals — "six seats", not "6 seats" — so EP004's hook (*"exactly one paying student … filled six seats"*) scored `signals: NONE` on a check whose entire purpose is detecting concrete tension. Fixed in **both** `eval_script.py` and `eval_package.py` (the logic was duplicated, and only fixing one left the other reporting the opposite verdict on the same hook). Deliberately conservative: a single spelled numeral is **not** a signal, because "one of the" would turn the check into a rubber stamp. Only **two or more distinct spelled quantities** count — a comparison, which is the thing the check is actually looking for. Also added `another / while / meanwhile` to the contrast markers. Validated against a bland control and a single-numeral control, both of which still correctly fail.

**Effect:** EP004 `small-cohort-business` went 0.604 → **0.700**, craft 43 → **57/63**, kill-list hit cleared, publish verdict BLOCKED → PROJECTED PASS — with no change to the script. EP005 `solo-design-agency` unchanged at 0.979 / 22/22 / 63/63. EP001–003 re-checked: full auto scores, no regressions.

**Standing note:** `tension_signals` is duplicated between `eval_script.py` and `eval_package.py`. Keep them in sync or merge them; a divergence here reports two different verdicts on the same hook.
