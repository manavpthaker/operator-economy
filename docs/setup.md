...
**VOICE v4 DECISION (2026-07-04): retrain Manav's PVC and use HIS voice.** Eight designed-voice rounds (N3 → NY-A1 → LW3 + performance pass) all passed 40s auditions and failed full episodes — designed TTS prosody isn't meaning-driven and the residue compounds over 8 minutes. The clone was muddy because of SAMPLES, not tech (diagnosis above). Plan: record 15–20 min narration-style (read the performed pilot script), clean via studio/tools/clean_voice_samples.sh, replace all old samples in the 'brown man pro' voice (`q9RtP62PWHxX4IPG7uSM`), retrain — voice_id survives retraining, so the config swap is one line. LW3 (`kfoyZeNUIHno5EcIjqO3`) stays as fallback. Performance pass + speech aliases + mastering are voice-agnostic and unchanged.
