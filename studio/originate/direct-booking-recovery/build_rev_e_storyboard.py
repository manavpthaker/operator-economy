...
TITLES = {
    ("thesis", 1): "Today: hotels keep paying to meet the same guest",
...
hook_cuts = [
    0.0,
    find_phrase("hook", "More than 60 percent"),
    find_phrase("hook", "A guest finds"),
    find_phrase("hook", "Today I'll show you"),
    hook_end,
]
...
screens.extend([
    screen(screen_id="hook-01", section="hook", layout="broll",
           ...
           title="A guest checks out. The relationship disappears.",
           body="A beautiful stay, then a hard break: the guest leaves and the property has to pay to meet them again.",
    ...
    screen(screen_id="hook-02", section="hook", layout="schematic",
           ...
           title="More than 60% arrives through OTAs",
           body="The platform creates reach. The hotel still has to earn the relationship.",
    ...
    screen(screen_id="hook-03", section="hook", layout="proof_card",
           ...
           title="A beautiful stay. Then the guest returns to Booking.",
           body="Discovery became a reservation, but the relationship stayed with the platform.",
           beat=1, source=hook_source, narrative_state="consequence", score_state="constraint",
           footage_role="proof", preview_eligible=True,
           custom={"proof": {"value": 2, "prefix": "Visit ",
                             "label": "The repeat booking",
                             "contrast": "The hotel delivered the stay; the platform kept the return path",
                             "estimate": False}}),
    screen(screen_id="hook-04", section="hook", layout="quote",
           start=hook_cuts[3], end=hook_cuts[4],
           title="Help the hotel earn the return visit directly.",
           body="THIS EPISODE · THE OPERATOR OUTCOME",
           beat=1, source=hook_source, narrative_state="recognition", score_state="silence",
           footage_role="outcome", preview_eligible=True,
           custom={"quote": "Help the hotel earn the return visit directly.",
                   "accentPhrase": "return visit directly", "ground": "navy"}),
])
