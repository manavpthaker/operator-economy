...
def load_broll_config() -> dict:
    config_path = Path(__file__).parents[2] / "config" / "default.json"
    if not config_path.exists():
        return {}
    return json.loads(config_path.read_text()).get("broll", {})
...
def candidate_score(video: dict, proxy_file: dict, query_rank: int) -> int:
    """Rank technical review fitness, not semantic fitness.

    Pexels does not return dependable scene labels for video. Query order carries
    the editorial preference; the remaining points only keep unusably short,
    portrait, or low-resolution media from floating to the top.
    """
    duration = video.get("duration", 0) or 0
    width = proxy_file.get("width", 0) or 0
    height = proxy_file.get("height", 0) or 0
    score = max(0, 4 - query_rank) * 100
    if 5 <= duration <= 30:
        score += 30
    elif 3 <= duration <= 60:
        score += 15
...
def contact_sheet(proxy: Path, output: Path, duration: float) -> None:
    # Six evenly distributed frames expose masks, scene changes, and weak
    # openings that a single poster image routinely hides.
    interval = max(float(duration) / 6, 0.25)
    subprocess.run([
        "ffmpeg", "-y", "-loglevel", "error", "-i", str(proxy),
        "-vf", f"fps=1/{interval:.3f},scale=360:-1,tile=3x2:padding=6:margin=6",
...
def main() -> None:
    ...
    ap.add_argument("--per-beat", type=int,
                    default=config.get("review_candidates_per_beat", 9),
                    help="review candidates retained per footage beat (default: 9)")
    ap.add_argument("--results-per-query", type=int,
                    default=config.get("results_per_query", 30),
                    help="Pexels results inspected for each query variant (default: 30, max: 80)")
    args = ap.parse_args()
    if not 1 <= args.per_beat <= 24:
        ap.error("--per-beat must be between 1 and 24")
    if not 1 <= args.results_per_query <= 80:
        ap.error("--results-per-query must be between 1 and 80")
    ...
    for entry in manifest["entries"]:
        ...
        for rank, query in enumerate(queries):
            ...
            url = "https://api.pexels.com/v1/videos/search?" + urllib.parse.urlencode({
                "query": query,
                "per_page": args.results_per_query,
            ...
            for video in request_json(url, key).get("videos", []):
                ...
                if not proxy_file:
                    continue
                score = candidate_score(video, proxy_file, rank)
        ...
        for candidate in chosen:
            ...
            if not sheet.exists():
                contact_sheet(proxy, sheet, candidate["duration_seconds"])
            candidate.update({"provider": "pexels", "license": LICENSE, "license_checked_at": date.today().isoformat(),
                              ...
                              "sha256": sha256(proxy), "review_status": "candidate",
                              "semantic_match": None, "faces_review": "needs_review",
                              "selected_in": None, "selected_out": None})
        payload["beats"].append({"id": entry["id"], "screen_id": entry.get("screen_id"), "role": entry["role"],
                                 ...
                                 "query_variants": queries, "visual_exclusions": entry.get("visual_exclusions", []), "candidates": chosen})
