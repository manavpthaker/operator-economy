...
CONSTRAINTS_GRAPHIC = (
    ...
    "consistent daylight across both. ONE hand and ONE object SHARP, and nothing "
    "else in the frame is sharp. Real material texture on both the skin and the "
    "object. Behind them the world continues, thrown well out of focus and low in "
    "contrast — soft shapes and colour rather than legible detail — so the "
    "silhouette still reads at a glance while the frame stops feeling empty. The "
    "upper third stays open and uncluttered for a text block. Every "
