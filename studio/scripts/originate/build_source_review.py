  ...
  <a href=\"{h(source['canonical_url'])}\" target=\"_blank\" rel=\"noreferrer\">Open canonical source ↗</a>
    ...
    for group in data["footage_candidates"]:
        ...
        for pexels_id in group["pexels_ids"]:
            ...
            cards.append(f'<div class="clip">{media}<strong>Pexels · {h(pexels_id)}</strong></div>')
        footage_groups.append(
            f"""<section class=\"footage-group\"><div class=\"group-head\"><h2>{h(group['asset_id'])}</h2><p>{h(group['review_note'])}</p></div><div class=\"clip-grid\">{''.join(cards)}</div></section>"""
        )
