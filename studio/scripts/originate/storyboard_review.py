...
def main() -> None:
    ...
    ap.add_argument("--render-data", help="Prepared render_data/blueprint.json for final episode timings")
    args = ap.parse_args()
    ...
    render_screens = {}
    if args.render_data:
        render_data = json.loads(Path(args.render_data).read_text())
        render_screens = {screen["id"]: screen for screen in render_data.get("screens", [])}
        # The prepared render data inserts a silent identity break and shifts
        # all downstream captions with it. Use those final word times so the
        # review describes the actual rendered episode, not its pre-insert
        # storyboard clock.
        caption_words = [
            word
            for group in render_data.get("captions", {}).get("groups", [])
    ...
    for index, screen in enumerate(data["screens"], 1):
        screen = render_screens.get(screen["id"], screen)
        ...
        if screen["id"] == "hook-04":
            # Follow the actual cold-open end.  The old fixed Rev D time made
            # the review report an alignment that no longer existed after a
            # hook rewrite and VO regeneration.
            ident_start = float(screen["end"])
            ident_end = ident_start + 4.0
            ...
            cards.append(f'''<article class="beat ident" data-role="identity" data-layout="ident">
              ...
              <div class="meta"><div class="time">{tc(ident_start)}–{tc(ident_end)}<small>4.0s identity break</small></div><div><h2>Logo sting → episode title</h2><h3>Voiceover</h3><blockquote>Intentional silence. The cold open resolves before the show logo appears.</blockquote><h3>Visual job</h3><p>Hard reset after the accusation. Establish The Operator Economy, name the episode, then begin the spoken “Today” announcement.</p></div></div>
    ...
    identity_after = next((screen for screen in data["screens"] if screen["id"] == "hook-04"), None)
    identity_note = (
        f"ident at {tc(float(identity_after['end']))}–{tc(float(identity_after['end']) + 4.0)}"
        if identity_after else "identity timing unavailable"
    )
    ...
    </style></head><body><header><div class="eyebrow">Operator Blueprint · № 011 · Script-aligned edit board</div><div class="brand">Hotels Keep Paying to Meet the Same Guest</div><div class="wave">{html.escape(states)}</div><div class="summary">31 authored screens · exact timed VO · actual Remotion frames · {identity_note}</div><nav><button class="active" data-filter="all">All {len(data['screens'])} screens</button><button data-filter="human_context">Human</button><button data-filter="proof">Proof</button><button data-filter="process">Process</button><button data-filter="outcome">Outcome</button><button data-filter="broll">B-roll</button><button data-filter="screen_rec">Screen capture</button></nav></header><main>{''.join(cards)}</main><script>
