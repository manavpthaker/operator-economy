#!/usr/bin/env python3
"""Render the canonical Operator Economy thesis Markdown to a designed PDF."""
...
OUTPUT = ROOT / "output" / "pdf" / "operator-economy-working-paper-02.pdf"
...
def draw_page(canvas, doc) -> None:
    canvas.saveState()
    canvas.setTitle("The Operator Economy — Working Paper No. 02")
    canvas.setAuthor("Manav Thaker / The Operator Economy")
    canvas.setSubject(
        "A working thesis of small-core firms, rented capability, and dependency-adjusted independence"
    )
    ...
    if doc.page == 1:
        ...
        canvas.drawString(0.74 * inch, PAGE_H - 0.82 * inch, "THE OPERATOR ECONOMY  /  WORKING PAPER 02")
        ...
        canvas.drawString(0.76 * inch, PAGE_H - 3.32 * inch, "SMALL CORE. THICK PERIMETER.")

        subtitle = Paragraph(
            "A working thesis of small-core firms, rented capability, "
            "and dependency-adjusted independence",
            ParagraphStyle(
                "cover-subtitle",
                fontName="OEArial",
                fontSize=15,
        ...
        canvas.drawString(0.76 * inch, 1.13 * inch, "RESEARCH DRAFT  /  24 AUGUST 2026")
        ...
        canvas.drawString(LEFT, PAGE_H - 0.36 * inch, "THE OPERATOR ECONOMY  /  WORKING PAPER 02")
        canvas.drawRightString(
            PAGE_W - RIGHT,
            PAGE_H - 0.36 * inch,
            "RESEARCH DRAFT  /  NOT CANONICAL DOCTRINE",
        )
...
class Diagram(Flowable):
    ...
    def draw_label(self, c, x, y, w, h, title, body="", fill=PAPER_LIGHT, stroke=RULE_STRONG, title_color=BLUE_DARK):
        c.setFillColor(fill)
...
def build() -> None:
    ...
    doc = ThesisDocTemplate(
        ...
        topMargin=TOP,
        bottomMargin=BOTTOM,
        title="The Operator Economy — Working Paper No. 02",
